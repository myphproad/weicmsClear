...
<link href="/weicmsClear/Public/static/font-awesome/css/font-awesome.min.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet">
<link href="/weicmsClear/Public/Home/css/base.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet">
<link href="/weicmsClear/Public/Home/css/module.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet">
<link href="/weicmsClear/Public/Home/css/weiphp.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet">
<link href="/weicmsClear/Public/static/emoji.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet">
<!-- Le HTML5 shim, for IE6-8 support of HTML5 elements -->
<!--[if lt IE 9]>
<script src="/weicmsClear/Public/static/bootstrap/js/html5shiv.js?v=<?php echo SITE_VERSION;?>"></script>
<![endif]-->
...
<!--<![endif]-->
<script type="text/javascript" src="/weicmsClear/Public/static/bootstrap/js/bootstrap.min.js"></script>
<script type="text/javascript" src="/weicmsClear/Public/static/uploadify/jquery.uploadify.min.js"></script>
<script type="text/javascript" src="/weicmsClear/Public/static/zclip/ZeroClipboard.min.js?v=<?php echo SITE_VERSION;?>"></script>
<script type="text/javascript" src="/weicmsClear/Public/Home/js/dialog.js?v=<?php echo SITE_VERSION;?>"></script>
<script type="text/javascript" src="/weicmsClear/Public/Home/js/admin_common.js?v=<?php echo SITE_VERSION;?>"></script>
<script type="text/javascript" src="/weicmsClear/Public/Home/js/admin_image.js?v=<?php echo SITE_VERSION;?>"></script>
<script type="text/javascript" src="/weicmsClear/Public/static/masonry/masonry.pkgd.min.js"></script>
<script type="text/javascript" src="/weicmsClear/Public/static/jquery.dragsort-0.5.2.min.js"></script> 
<script type="text/javascript">
var  IMG_PATH = "/weicmsClear/Public/Home/images";
...
<div class="navbar navbar-inverse navbar-fixed-top">
    <div class="wrap">
       ...
       <?php if(!empty($userInfo[website_logo])): ?><img height="52" src="<?php echo (get_cover_url($userInfo["website_logo"])); ?>"/>
       	<?php else: ?>
       		<img height="52" src="/weicmsClear/Public/Home/images/logo.png"/><?php endif; ?>
       </a>
                ...
                <?php if(is_login()): ?><ul class="nav" style="margin-right:0">
                        ...
                        <li class="dropdown admin_nav">
                            <a href="#" class="dropdown-toggle login-nav" data-toggle="dropdown" style="">
                                ...
                                <?php else: ?>
                                    <img class="admin_head default" src="/weicmsClear/Public/Home/images/default.png"/><?php endif; ?>
      ...
      <ul class="sidenav">
        <li>
          <?php if(!empty($now_top_menu_name)): ?><a class="sidenav_parent" href="javascript:;"> 
            <!--<img src="/weicmsClear/Public/Home/images/left_icon_<?php echo ($core_side_category["left_icon"]); ?>.png"/>--> 
        ...
        <?php if(!empty($addonList)): ?><li> <a class="sidenav_parent" href="javascript:;"> <img src="/weicmsClear/Public/Home/images/ico1.png"/> 其它功能</a>
...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
            <div class="table-bar">
                <div class="fl">
                    <div class="tools">
                        <a href="javascript:void(0);" class="btn setting_group">设置用户组</a> &nbsp;
                        <a href="javascript:void(0);" class="btn send_salary">批量发工资</a> &nbsp;
                ...
                <div class="search-form fr cf">
                    <div class="sleft" style="margin-right:10px;">
                        <select name="group" style="border:none; padding:4px; margin:0;">
                            <option value="<?php echo addons_url('UserCenter://UserCenter/lists',array('group_id'=>0));?>"
                            <?php if(($$group_id) == "0"): ?>selected<?php endif; ?>
                            >全部用户</option>
                            <?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo addons_url('UserCenter://UserCenter/lists',array('group_id'=>$vo['id']));?>"
                                <?php if(($vo['id']) == $group_id): ?>selected<?php endif; ?>
                                ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </div>
                    <div class="sleft">
                        <input type="text" placeholder="请输入昵称或手机号搜索" value="<?php echo I('nickname');?>" class="search-input"
                               name="nickname">
                        <a url="<?php echo addons_url('UserCenter://UserCenter/lists',$get_param);?>" id="search"
                           href="javascript:;" class="sch-btn"><i class="btn-search"></i></a></div>
                </div>
                <!-- 多维过滤 -->
            </div>
            <!-- 数据列表 -->
                ...
                <div class="table-striped">
                    <table cellspacing="1">
                        ...
                        <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                <td><input type="checkbox" id="check_<?php echo ($vo["uid"]); ?>" name="ids[]" value="<?php echo ($vo["uid"]); ?>"
                                ...
                                <td>
                                    <?php if(empty($vo["group"])): ?>暂无分组
                                        <?php else: ?>
                                        <?php echo ($vo["group"]); endif; ?>
                                    </td>
                                <!-- <td><?php echo ($vo["tag_titles"]); ?></td>-->
                                <td>
                                    <a href="<?php echo addons_url('UserCenter://UserCenter/detail',array('uid'=>$vo[uid]));?>"
                                       target="_self">详细资料</a>
                                  <!--  <a href="<?php echo addons_url('UserCenter://UserCenter/edit',array('uid'=>$vo[uid]));?>"
                                       target="_self">编辑</a>-->
                                    ...
                                    <?php if($vo[is_agent]): ?><a href="<?php echo U('set_agent',array('uid'=>$vo[uid],'is_agent'=>0));?>" >解除代理</a>
                                    <?php else: ?>
                                        <a href="<?php echo U('set_agent',array('uid'=>$vo[uid],'is_agent'=>1));?>" >设置代理</a><?php endif; ?>
                                </td>
    ...
    <div class="group_html" style="display:none">
        <div class="manage_group normal_dialog">
            <div class="content">
                ...
                <div class="btn_wrap">
                    <button class="btn setting_group" url="<?php echo U('changeGroup');?>" target-form="ids">确定</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="remark_html" style="display:none">
        <div class="manage_group normal_dialog">
            <div class="content">
                ...
                <div class="btn_wrap">
                    <button class="btn setting_remark" url="<?php echo U('set_remark');?>">确定</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <p class="copyright">本系统由<a href="http://www.eciyuan.net" target="_blank">亿次元科技</a>助力开发</p>
</div>
        ...
        $(function () {
            ...
            $('.setting_tag').click(function () {
                ...
                var html = $('<div class="manage_group normal_dialog">' +
                        ...
                        '<div class="btn_wrap"><button class="btn setting_tag" url="<?php echo U('
                changeTag
                ');?>" target-form="ids">确定</button></div>' +
                '</div>' +
                '</div>'
                )
                ;
