...
class UserAgentController extends AddonsController
{
    ...
    public function lists()
    {
        ...
        if ($agent['is_agent'] == 0) $this->error('你无开通代理权限');
...
//            dump($list_data);
    ...
    public function userBondLists()
    {
        ...
        if (!$is_agent) $this->error('非代理人');

        //得到该代理人下的所有用户
        $uids = $this->getUids($uid);
        if(empty($uids)) $this->error('没有用户数据');

        $bond_map['user_id'] = array('in', $uids);

        $model = $this->getModel('user_bond_logs');
        $page = I('p', 1, 'intval'); // 默认显示第一页数据
        $row = 5;
        // 解析列表规则
        $list_data = $this->_get_model_list($model);
        //用户 保证金金额 状态 创建时间
        $data = M('user_bond_logs')
            ->where($bond_map)
            ->page($page, $row)
            ->select();
//        if (!$data) $this->error('无数据');
        foreach ($data as $key => $value) {
            if ($value['user_id']) {
                $data[$key]['user_name'] = get_nickname($value['user_id']);
            } else {
                $data[$key]['user_name'] = use_openid_get_name($value['openid']);
            }
            if (0 == $value['status']) {
                $data[$key]['str_status'] = '未缴纳';
            } elseif (1 == $value['status']) {
                $data[$key]['str_status'] = '已缴纳';
            } elseif (3 == $value['status']) {
                $data[$key]['str_status'] = '保证金已退还';
            }
        }
        /* 查询记录总数 */
        $count = M('user_bond_logs')->where($bond_map)->count();
        $list_data ['list_data'] = $data;
        // 分页
        if (intval($count) > $row) {
            $page = new \Think\Page ($count, $row);
    ...
    public function jobSubscribeLists()
    {
        ...
        /* 查询记录总数 */
        $count = M('job_subscribe')->where($job_map)->count();
        $list_data ['list_data'] = $info;
        // 分页
        if (intval($count) > $row) {
            $page = new \Think\Page ($count, $row);
            $page->setConfig('theme', '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
...
//        dump($info);exit();
        ...
        foreach ($info as $key => $value) {
            if (empty($value['user_id'])) {
                $info[$key]['user_id'] = use_openid_get_name($value['openid']);//openid获取用户
            } else {
                $info[$key]['user_id'] = get_nickname($value['user_id']);
            }
            $info[$key]['job_id'] = get_about_name($data[$value['job_id']], 'job', 'title');
        }
        /* 查询记录总数 */
        $count = M('job_subscribe')->where($job_map)->count();
        $list_data ['list_data'] = $info;
