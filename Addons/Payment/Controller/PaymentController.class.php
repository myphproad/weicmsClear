...
//			dump($_POST ["wx_cert_pem"]);
			if ($_POST ["wx_cert_pem"]=="*" ||$_POST ["wx_cert_pem"]=='') {
			    unset ( $_POST ["wx_cert_pem"] );
			}
			if ($_POST ["wx_key_pem"]=="*"||$_POST ["wx_key_pem"]=='') {
			    unset ( $_POST ["wx_key_pem"] );
			}
			if ($_POST ["wx_root_pem"]=="*"||$_POST ["wx_root_pem"]=='') {
...
//			dump($data);
			if ($data['wx_cert_pem'] || $data['wx_key_pem'] || $data['wx_root_pem']){
			    $certFile = M ( 'file' )->where(array('id'=>$data['wx_cert_pem']))->getfield('name id');
			    $keyFile = M ( 'file' )->where(array('id'=>$data['wx_key_pem']))->getfield('name id');
			    $rootFile = M ( 'file' )->where(array('id'=>$data['wx_root_pem']))->getfield('name id');
